...
Button::Button(sf::Vector2i& mousePosition, sf::Vector2i position, std::shared_ptr<sf::Image> idleImage,
    ...
    : IClickable(position, static_cast<sf::Vector2i>(idleImage->getSize()), mousePosition) {
...
void Button::update() { updateClickable(); }

void Button::render(sf::RenderTarget &target) { target.draw(*m_currentSprite); }
